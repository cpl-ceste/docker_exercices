...
Y hay más razones. Entonces, para aplicaciones con multiples componentes como en la imagen siguiente, es mejor ejecutar la aplicación de forma que los componentes se orquesten independientemente en diferentes contenedores 
...
- Luego levantamos un contendor mysql en esa red y le asignamos un alias DNS en esa red `mysql`
`$ docker run -d \
...
- Finalmente y para ver como se ha levantado en la red, vamos a usar un contenedor que tiene una serie de herramientas de red entre ellas el comando `dig` para resolver nombres de dominio en la red. Para inspeccionar si el alias `mysql` se resuelve correctamente en la red `todo-app`
...
El comando dig (Domain Information Groper) es una herramienta de red que sirve para consultar DNS, es decir, para traducir nombres (como `mysql`) a direcciones IP. Vemos que en el DNS de la red se resuelve ok este nombre de dominio, que es un registro de DNS de tipo ALIAS A para el nombre -> mysql. Asi que no hace falta que usemos la ip de la red para conectarnos a la Base de Datos desde otro contenedor, usaremos el alias.
